...
void OXRender::composite(OXPicture src, OXPicture mask, const QPixmap &dst,
                         int sx, int sy, int mx, int my, int dx, int dy,
                         uint w, uint h, int op) {
   XRenderComposite (dpy, op, src, mask, dst.x11PictureHandle(),
...
QPixmap OXRender::applyAlpha(const QPixmap &toThisPix,
...
void OXRender::setAlpha(QPixmap &pix, const OXPicture &alpha) {
   ...
   XRenderChangePicture(dpy, pix.x11PictureHandle(),
                        CPAlphaMap|CPAlphaXOrigin|CPAlphaYOrigin, &pa);
}
...
void OXRender::setColor(XRenderColor &xc,
