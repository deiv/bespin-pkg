...
//    bool scrollerActive = false;
   ...
   while (scrollWidget &&
          ...
          qobject_cast<Q3ScrollView*>(scrollWidget) ||
           _scrollAreas.contains(scrollWidget)))
         ...
         if ((cmb->subControls & SC_ComboBoxFrame) && cmb->frame) {
            if (cmb->editable) {
               ...
               if (!isEnabled || ar.isNull()) {
                  ...
                  if (listShown) step = 6;
                  c = btnBgColor(PAL, isEnabled, hasFocus, 0);
                  ...
                  if (hasFocus)
                     masks.tab.outline(r, painter, midColor(COLOR(Window),
                  ...
                  if (!config.fullButtonHover && step) { // jupp ;)
                     ...
                     masks.tab.render(r, painter, Gradients::brush(
                        midColor(c, COLOR(Button), 6-step, step), RECT.height()-f2,
...
//    case CC_Q3ListView: // Used for drawing the Q3ListView class
   case CC_Dial: // A dial, like QDial
      if (const QStyleOptionSlider *dial =
         ...
         painter->setPen(midColor(PAL.background().color(), PAL.foreground().color()));
